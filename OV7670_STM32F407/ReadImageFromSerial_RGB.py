...
def bytes2image(recv_bytes):
    ...
    raw_data_rgb = [[((i & 0xFFFF) >> 11) << 3, ((i & 0x7FF) >> 5) << 2, (i & 0x1F) << 3] for i in buff_list_hex]
    np_data = np.array(raw_data_rgb).astype('uint8')
    # print("np_data:")
    # print(np_data)
    # print("np_data shape:", np_data.shape)

    display_data = np_data.reshape(H, W, 3)
    display_data = np.flip(display_data, 0)  # [0:240) to [240:0)
